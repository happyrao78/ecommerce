...
import { Pencil, Trash2, PlusCircle, X, Check, Info, Search, Filter } from "lucide-react";
...
const ListDiscount = () => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Discount Coupons</h2>
        <p className="text-gray-600">Manage your store's discount coupons and promotions</p>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <Link to="/add-discount" className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 transition duration-200 flex items-center gap-2">
          <PlusCircle size={18} />
          Add New Coupon
        </Link>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{new Date(viewData.startDate).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">End Date</p>
                  <p className="font-medium">{new Date(viewData.endDate).toLocaleDateString()}</p>
                </div>
              </div>
